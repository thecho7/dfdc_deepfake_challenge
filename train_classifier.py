...
def main():
	...
	arg('--from-zero', action='store_true', default=True)
	...
	arg("--logger", type=str, default='vessl')

	# Training Hyperparameters
	arg("--network", type=str, default="DeepFakeClassifier")
	arg("--encoder", type=str, default="tf_efficientnet_b7_ns")
	arg("--epochs", type=int, default=20)
	arg("--batches-per-epoch", type=int, default=2500)
	arg("--size", type=int, default=380)
	arg("--fp16", action='store_true', default=True)
	arg("--batch-size", type=int, default=4)
	arg("--optimizer", type=str, default="SGD")
	arg("--momentum", type=float, default=0.9)
	arg("--weight-decay", type=float, default=1e-4)
	arg("--learning-rate", type=float, default=1e-4)
	arg("--nesterov", action='store_true', default=True)
	arg("--scheduler-type", type=str, default="poly")
	arg("--scheduler-mode", type=str, default="step")
	...
	# Overwritting an optimizer config (To use sweep)
	conf = load_config(args.config)
	conf["optimizer"]["batch_size"] = args.batch_size
	conf["optimizer"]["type"] = args.optimizer
	...
	conf["optimizer"]["schedule"]["type"] = args.scheduler_type
	conf["optimizer"]["schedule"]["mode"] = args.scheduler_mode
	conf["optimizer"]["schedule"]["epochs"] = args.epochs
	print(conf)
	...
	if args.local_rank == 0:
		...
		if args.logger == 'wandb':
			logger = wandb.init(config=args, project='deepfake', notes=f'Batch size {batch_size}')
			logger.watch(model)
		## vessl logger
		elif args.logger == 'vessl':
			vessl.init(hp=args, project='interview', message=f'Batch size {batch_size}')
			logger = None
	else:
		logger = None
	...
	for epoch in range(start_epoch, max_epochs):
		...
		# Add wandb logger
		train_epoch(args, current_epoch, loss_functions, model, optimizer, scheduler, train_data_loader, logger, conf,
...
def evaluate_val(args, data_val, bce_best, model, snapshot_name, current_epoch, logger):
	print("Test phase")
	model = model.eval()
	...
	if args.local_rank == 0:
		...
		result_targets = np.asarray(data_y)
		result_preds= np.asarray(data_x)
		result_targets[result_targets >= 0.5] = 1
		result_preds = np.where(result_preds >= 0.5, 1, 0)
...
def train_epoch(args, current_epoch, loss_functions, model, optimizer, scheduler, train_data_loader, logger, conf,
	...
	if local_rank == 0:
		if args.logger == 'wandb':
			logger.log({"lr": float(scheduler.get_lr()[-1]), "train/total_loss": losses.avg,
						"train/fake_loss": fake_losses.avg, "train/real_loss": real_losses.avg}, commit=False)
		elif args.logger == 'vessl':
			vessl.log({"lr": float(scheduler.get_lr()[-1]), "train/total_loss": losses.avg,
						"train/fake_loss": fake_losses.avg, "train/real_loss": real_losses.avg}, step=current_epoch)
